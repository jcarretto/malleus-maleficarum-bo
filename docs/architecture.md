...
| Fraud and tampering | Owner decisions of 2026-09-25, implemented by the tasks named here. The Premium entitlement lives 1 month instead of 1 year (API-66), which bounds offline use after a revocation. Back-office access tokens are rejected on player routes (API-67). Admins can revoke a user's Premium with a reason (API-68, BO-33) and void activation codes (API-69, BO-22). The entitlement is bound to the installation with an `iid` claim (API-70, PWA-36), so a copied entitlement does not work on another install. The PWA accepts only entitlement keys pinned in its build (PWA-35), so a key written into IndexedDB never verifies; a new signing key is pinned in a PWA release before the API starts signing with it. Kept as is: admin sessions of 12 hours sliding with no absolute cap, and admin sign-in throttled per IP only. Residual risk: someone who modifies the app's code on their own device can bypass any client check; Premium content served by the API stays protected by the API. |
